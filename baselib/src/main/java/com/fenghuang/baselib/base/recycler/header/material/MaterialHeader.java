...
public class MaterialHeader extends InternalAbstract implements RefreshHeader {
    ...
    public MaterialHeader(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mProgress = new MaterialProgressDrawable(thisView);
        mProgress.setBackgroundColor(CIRCLE_BG_LIGHT);
        mProgress.setAlpha(255);
        mProgress.setColorSchemeColors(getThemeColor(context));

        mCircleView = new MaterialImageView(context, CIRCLE_BG_LIGHT);
...
//                resolveSize(getSuggestedMinimumHeight(), heightMeasureSpec));
    ...
    protected void dispatchDraw(Canvas canvas) {
        if (mShowBezierWave) {
            ...
            //绘制贝塞尔曲线
            final View thisView = this;
            mBezierPath.quadTo(thisView.getMeasuredWidth() * 1f / 2, mHeadHeight + mWaveHeight * 1.9f, thisView.getMeasuredWidth(), mHeadHeight);
...
//            kernel.requestDefaultHeaderTranslationContent(false);
    ...
    public void onMoving(boolean isDragging, float percent, int offset, int height, int maxDragHeight) {
        ...
        if (isDragging || (!mProgress.isRunning() && !mFinished)) {
            ...
            float targetY = offset * 1f / 2 + mCircleDiameter * 1f / 2;
...
//    }
    ...
    public void onReleased(@NonNull RefreshLayout layout, int height, int maxDragHeight) {
        ...
        if ((int) circleView.getTranslationY() != height / 2 + mCircleDiameter / 2) {
            circleView.animate().translationY(height * 1f / 2 + mCircleDiameter * 1f / 2);
        }
    }
...
//    }
    ...
    private int getThemeColor(Context context) {
        ...
        TypedArray typedArray = theme.obtainStyledAttributes(array);
