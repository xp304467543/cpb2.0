...
public class ImageLoader {
    ...
    public static void loadImage(SimpleDraweeView simpleDraweeView, Bitmap bitmap, Context context) {
        if (bitmap == null || simpleDraweeView == null) {
            return;
        }
        Uri uri = null;
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            uri = Uri.parse(MediaStore.Images.Media.insertImage(AppUtils.INSTANCE.getApplication().getContentResolver(), bitmap, null, null));
        } else {
            uri = addPictureToAlbum(context, bitmap, "crop_img");
        }
        loadImage(simpleDraweeView, uri, 0, 0, null, null, false);
    }
